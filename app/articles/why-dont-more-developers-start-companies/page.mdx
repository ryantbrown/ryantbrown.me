...
export const article = {
  author: 'Ryan Brown',
  date: '2024-07-22',
  title: 'Why don\'t more developers build product companies?',
  description:
...
```ts
export async function resolve<T extends StorageValue>(
  key: string,
  fallback: () => Promise<T>,
  options = { ttl: 60 }
): Promise<T> {
  try {
    const cached = await kv.getItem<T>(key);
    ...
    await kv.setItem(key, value, options);
